...
const getTempVals = (town, str) => {
  const townVals = str
    .split("\n")
    .filter((e) => e.slice(0, town.length) === town);
  return townVals.length === 0
    ? [-12]
    : townVals[0]
        .slice(town.length + 1)
        .split(",")
...
const mean = (town, str) => {
  return getTempVals(town, str).reduce((acc, cv) => acc + cv) / 12;
};

const variance = (town, str) => {
  const dataVals = getTempVals(town, str);
  if (dataVals.length === 1) return -1;
  const meanVal = mean(town, str);

  return (
    dataVals
      .map((month, i) => (month - meanVal) ** 2)
      .reduce((acc, cv) => acc + cv) / 12
  );
};

const assertCloseTo = (actual, expected, epsilon = 0.01) => {
  if (Math.abs(actual - expected) <= epsilon) {
    return true;
  } else {
    console.log(`expected ${expected} to be close to ${actual} +/- ${epsilon}`);
    return false;
  }
};

console.log("rainfall mean, data");
console.log(assertCloseTo(mean("London", data), 51.199999999999996));
console.log("rainfall mean, data1");
console.log(assertCloseTo(mean("London", data1), 57.03333333333333));
console.log("rainfall variance, data");
console.log(assertCloseTo(variance("London", data), 57.428333333333335));
console.log("rainfall variance, data1");
console.log(assertCloseTo(variance("London", data1), 110.90055555555556));

// console.log(mean("Berlin", data));
// console.log(mean("Beijing", data));
// console.log(variance("London", data));

// console.log(getTempVals("London", data));
// console.log(getTempVals("London", data1));
